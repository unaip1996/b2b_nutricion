...
import { Brain, Users, BookOpen, UserCog, User, LogOut, ChevronUp } from "lucide-react"
...
export function ClinicalSidebar() {
  ...
  const navigation = [
    { name: "Motor RAG Clínico", href: "/dashboard/rag", icon: Brain, show: true },
    { name: "Directorio Pacientes", href: "/dashboard/patients", icon: Users, show: true },
    { name: "Base de Conocimiento", href: "/dashboard/knowledge-base", icon: BookOpen, show: true },
    { name: "Directorio Usuarios", href: "/dashboard/users", icon: UserCog, show: isAdmin }, // Protegido
  ]
  ...
  return (
    <aside className="flex w-64 flex-col bg-slate-900">
      ...
      <div className="relative mt-auto border-t border-slate-800 p-4" ref={dropdownRef}>
        ...
        >
          ...
          <div className="flex-1 overflow-hidden">
            <p className="truncate text-sm font-medium text-slate-200">
                {userEmail !== "Cargando..." ? userEmail.split('@')[0] : "Dr. Facultativo"}
            </p>
            <p className="text-xs text-slate-500">Licencia Activa</p>
          </div>
          <ChevronUp className={`h-4 w-4 text-slate-500 transition-transform ${isDropdownOpen ? "rotate-180" : ""}`} />
        </button>
      </div>
    </aside>
  )
